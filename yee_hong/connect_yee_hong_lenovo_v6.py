...
def check_internet_connection(url):
    ...
    try:
        a=requests.get(url)
        if(a.url==url):
            print(current_time,"Connected.", file=open(date_today+"_output.txt","a"))
            # print("connected")
            time.sleep(10)
            global fail_count
            fail_count=0
            ...
            print(current_time,"No internet connection.",file=open(date_today+"_output.txt","a"))
            return(False)
    except:
        print(current_time,"Failure in checking internet connection.",file=open(date_today+"_output.txt","a"))
        return(False)
...
def reset_network_adapter(five_ghz_adapter_name):
    ...
    current_time = now.strftime("%H:%M:%S")
    try:
        print(current_time,"Trying to reset the connection.",file=open(date_today+"_output.txt","a"))
        os.system('netsh interface set interface name="'+five_ghz_adapter_name+'" admin=DISABLED &')
        print(current_time,"adapter disabled "+datetime.now().strftime("%H:%M:%S"),file=open(date_today+"_output.txt","a"))
        time.sleep(15)
        os.system('netsh interface set interface name="'+five_ghz_adapter_name+'" admin=ENABLED &')
        print(current_time,"adapter enabled "+datetime.now().strftime("%H:%M:%S"),file=open(date_today+"_output.txt","a"))
        time.sleep(15)
        return(True)
    except:
        print(current_time,"Failure of sending network adapter reset command, or to restore connectivity.", file=open(date_today+"_output.txt","a"))
        return(False)
...
def connect_to_network(five_ghz_adapter_name, ssid, name):
    ...
    current_time = now.strftime("%H:%M:%S")
    try:
        print(current_time,"Trying to connect to network with ssid: ",ssid,".",file=open(date_today+"_output.txt","a"))
        os.system('netsh wlan connect ssid='+ssid+' name='+name+' interface="'+five_ghz_adapter_name+'" &')
        time.sleep(10)
        return(True)
    except:
        print(current_time,"Failure of sending command to connect to network with ssid ",ssid,".", file=open(date_today+"_output.txt","a"))
        return(False)
...
def turn_on_hotspot(run_hotspot,path_to_turn_ON_hotspot_script):
    ...
    try:
        if (run_hotspot):
            print(current_time, "turning on hotspot...",file=open(date_today+"_output.txt","a"))
            os.system('powershell -File "'+path_to_turn_ON_hotspot_script+'" &')
            return(True)
    except:
        print(current_time,"Failure to send hotspot ON command.", file=open(date_today+"_output.txt","a"))
        return(False)
...
def turn_off_hotspot(path_to_turn_OFF_hotspot_script):
    ...
    current_time = now.strftime("%H:%M:%S")
    try:
        print(current_time,"turning off hotspot...",file=open(date_today+"_output.txt","a"))
        os.system('powershell -File "'+path_to_turn_OFF_hotspot_script+'" &')
        return(True)
    except:
        print(current_time,"Failure to send hotspot OFF command.", file=open(date_today+"_output.txt","a"))
        return(False)
...
def accept_yee_hong_agreement(url,chrome_options):
    ...
    current_time = now.strftime("%H:%M:%S")
    try:
        print(current_time,"accepting yee hong agreement...",file=open(date_today+"_output.txt","a"))
        ...
        print(current_time,"Failure to accept yee hong agreement.", file=open(date_today+"_output.txt","a"))
        return(False)
...
def check_if_reboot_needed(path_reboot_bat):
    ...
    try:
        ...
        print(current_time,"fail count is..."+str(fail_count),file=open(date_today+"_output.txt","a"))
        if(fail_count>10):
            fail_count=0
            os.system(path_reboot_bat+' &')
    except:
        print(current_time,"Failure to send reboot command.", file=open(date_today+"_output.txt","a"))
...
while True:
    ...
    if (not check_internet_connection(url)):
        ...
        if(not check_internet_connection(url)):
            ...
            if(not check_internet_connection(url)):
                check_if_reboot_needed(fail_count=fail_count,path_reboot_bat=path_reboot_bat)
                turn_off_hotspot(path_to_turn_OFF_hotspot_script=path_to_turn_OFF_hotspot_script)
                connect_to_network(five_ghz_adapter_name=five_ghz_adapter_name, ssid="dragon_backup", name="dragon_backup")
                turn_on_hotspot(run_hotspot=run_hotspot,path_to_turn_ON_hotspot_script=path_to_turn_ON_hotspot_script)
