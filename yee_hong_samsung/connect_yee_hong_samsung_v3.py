...
while True:
    try:
        ...
        try:
            ...
            print("adapter disabled",file=open(date_today+"_output.txt","a"))
            time.sleep(15)
            os.system('netsh interface set interface name="'+five_ghz_adapter_name+'" admin=ENABLED')
            print("adapter enabled",file=open(date_today+"_output.txt","a"))
            time.sleep(15)
            ...
            if(b.url==url):
                ...
                raise ValueError("we are being redirected!!",file=open(date_today+"_output.txt","a"))
            now = datetime.now()
            current_time = now.strftime("%H:%M:%S")
            print("Connected. Current Time =", current_time,file=open(date_today+"_output.txt","a"))
            if (run_hotspot):
